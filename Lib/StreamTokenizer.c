...
StreamTokenizer *StreamTokenizerInit(char *s) {
    ...
    // 不要在这里调用 StreamTokenizerType，因为它会重新初始化 tokenId
    // 而是在需要时单独调用
...
void StreamTokenizerTokenIDAdd(StreamTokenizerTokenID *tokenID, int pos, DBTokenType type) {
    // 检查是否需要扩展容量
...
void StreamTokenizerTokenIDModify(StreamTokenizerTokenID *tokenID, int length,int pos, DBTokenType type) {
...
//            streamTokenizer->streamTokenizerTokenId->len++;
    ...
    // 重置len为0，以便StreamTokenizerNext函数从第一个token开始处理
...
int StreamTokenizerNext(StreamTokenizer *streamTokenizer){
    // 确保首先调用了StreamTokenizerType来初始化token数据
    if (streamTokenizer->streamTokenizerTokenId->endLen == 0) {
        // 如果token数据未初始化，调用StreamTokenizerType初始化
        StreamTokenizerType(streamTokenizer);
    }
    
    if (streamTokenizer->streamTokenizerTokenId->len >= streamTokenizer->streamTokenizerTokenId->endLen) {
        // 已经到达末尾
        streamTokenizer->type = TOKEN_END_OF;
        return 0; // TOKEN_END_OF
    }
    
    int currentIndex = streamTokenizer->streamTokenizerTokenId->len;
    int startPos = streamTokenizer->streamTokenizerTokenId->data[currentIndex].pos;
    DBTokenType startType = streamTokenizer->streamTokenizerTokenId->data[currentIndex].type;
    
    if(startType == TOKEN_WORD){
        int len = streamTokenizer->streamTokenizerTokenId->data[currentIndex].len;
        if (streamTokenizer->sValue != NULL) {
            free(streamTokenizer->sValue); // 防止内存泄漏
        }
        streamTokenizer->sValue = malloc(len + 1); // 为字符串分配内存
        if (streamTokenizer->sValue) {
        ...
        if (str1) {
            ...
            str1[len] = '\0'; // 添加终止符
            streamTokenizer->iValue = atoi(str1); // 使用atoi而不是atof，因为是整数
            free(str1); // 释放内存
        }
        streamTokenizer->type = TOKEN_INT_CONSTANT;
        streamTokenizer->streamTokenizerTokenId->len++;
        return -2; // TOKEN_INT_CONSTANT
    }else if(startType == TOKEN_DELIM){
        int len = streamTokenizer->streamTokenizerTokenId->data[currentIndex].len;
        if (streamTokenizer->sValue != NULL) {
            free(streamTokenizer->sValue); // 防止内存泄漏
        }
        ...
        streamTokenizer->streamTokenizerTokenId->len++;
        // 返回分隔符字符
        return streamTokenizer->data[startPos];
    }else if(startType == TOKEN_UNKNOWN){
        int len = streamTokenizer->streamTokenizerTokenId->data[currentIndex].len;
        if (streamTokenizer->sValue != NULL) {
            free(streamTokenizer->sValue); // 防止内存泄漏
        }
        ...
        // 跳过其他类型（如TOKEN_NULL）
        streamTokenizer->streamTokenizerTokenId->len++;
        return StreamTokenizerNext(streamTokenizer);
    }
}