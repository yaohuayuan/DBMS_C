...
int RecordPageSearchAfter(RecordPage*recordPage, int slot, int flag) {
    slot++;
//    printf("DEBUG: RecordPageSearchAfter - 开始搜索从slot %d, 查找flag %d\n", slot, flag);
...
//        printf("DEBUG: slot %d, offset %d, 读取到的flag %d, 期望flag %d\n",
//               slot, offset, currentFlag, flag);

        if(currentFlag == flag) {
//            printf("DEBUG: 找到匹配的slot %d\n", slot);
            return slot;
        }
        slot++;
    }
//    printf("DEBUG: 没有找到匹配的slot\n");
    return -1;
}
void RecordPageFormat(RecordPage *recordPage){
    ...
    while (RecordPageIsValidSlot(recordPage,slot)){
        ...
        while (fieldNode!=NULL){
            int offset = RecordPageOffset(recordPage, slot);
//            printf("DEBUG: 格式化slot %d, offset %d, 设置flag为 %d\n",
...
//    printf("DEBUG: RecordPageFormat - 格式化完成\n");
}
