...
Buffer* BufferListGetBuffer(BufferList *bufferList,BlockID *blockId){
    BufferTEMP *bufferTemp= map_get(bufferList->buffers, CStringGetPtr(BlockID2CString(blockId)) );
...
void BufferListPin(BufferList *bufferList,BlockID *blockId){
    ...
    map_set(bufferList->buffers,CStringGetPtr(BlockID2CString(blockId)),*bufferTemp);
    ...
    NewBlockIDNode->blockId = BlockIDInit(blockId->fileName,blockId->BlockID);
...
void BufferListUnPin(BufferList *bufferList,BlockID *blockId){
    ...
    if(!flag){
        map_remove(bufferList->buffers, CStringGetPtr(BlockID2CString(blockId)));
    }
}
void BufferListUnpinAll(BufferList *bufferList) {
    BlockIDNode *p = bufferList->pin->next, *temp;
    while (p) {
        BufferTEMP *bufferTemp = map_get(bufferList->buffers, CStringGetPtr(BlockID2CString(p->blockId)));
        Buffer *buffer = bufferTemp->buffer;
        BufferManagerUnpin(bufferList->bufferManager, buffer);
        p = p->next;
    }
